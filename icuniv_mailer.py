...
<body>
    ...
    def send_email(self, to_email: str, subject: str, html_content: str, logo_path: str = "logo.png") -> tuple:
        """Send HTML email with embedded logo using Gmail API and return (success, message_id)"""
        try:
            ...
            for idx, prof in df.iterrows():
                ...
                try:
                    ...
                    with timeout_handler(150):
                        ...
                        email_success, message_id = self.send_email(prof['email'], subject, html_body, "logo.png")
                        
                        df.at[idx, 'email_sent'] = email_success
                        if email_success and message_id:
                            df.at[idx, 'message_id'] = message_id
                            sent_count += 1
                        else:
                            failed_count += 1
